...
| `Notification` | 入力待ちなどが 6〜60 秒続いた | `入力待ち` など型ごとの日本語。`permission_prompt` は直前が許可待ちの行なら重ねない。`auth_success` や `agent_completed` のような待ちでない型は書かない。**`入力待ち` は「詰まっている」ではなく「終わって次を待っている」として扱う**（#438。要対応では下段に出て、バッジにも通知にも数えない） |
...
- **SAI は外に出さない。** `127.0.0.1` 限定。デプロイもホスティングもしない。中身は作業内容そのもの。出してよいのは **tailnet まで**で、それも `tailscale serve`（前段のプロキシ）経由だけ。アプリ自身の bind は変えないし、`tailscale funnel` は使わない。Serve のヘッダは `whois` で突き合わせ、合わなければ `401`。タグ付きの端末（Serve が身元のヘッダを付けない）は画面を使えず、tailnet の ACL で許したときだけ MCP（`/mcp`）を呼べる。**MCP のツールで読んだ本文は、呼んだ側のエージェントの LLM に送られる**（[docs/tailnet.md](docs/tailnet.md)）。**例外は `JEV_API_KEY` を置いたときの許可の予想**（#491）で、許可の要約・コマンド・理由が TypeSafe AI に送られる（自分のメニューで切れる）。さらに自分のメニューで「自動で常に許可」の閾値を選ぶと（#499。既定はしない）、**外のモデルの判定で Claude の返信の Bash の許可が自動で通り、ルールが cwd の `.claude/settings.local.json` に書かれる**（この回のコマンドとルールの両方が閾値以上のときだけ）
...
| `JEV_API_KEY` | 許可のバブルに「許可して問題なさそうか」の確率を出すときの、Jev（TypeSafe AI）の鍵（#491）。**あるときだけ、許可のツール名・コマンド・パス・理由を TypeSafe AI に送る**（ファイルやメッセージの本文と cwd は送らない）。自分のメニューの「許可を Jev で予想する」で切れる（既定は入）。画面からは変えられない。「自動で常に許可」（#499。既定はしない）を入にすると、閾値以上の Claude の Bash の許可を自動で通す |
...
| `SAI_GH` | `0` で差分ボタンの PR 番号を引かない（既定は引く。SAI が外のネットワークに問い合わせる所の 1 つ。もう 1 つは `JEV_API_KEY`）。叩くのは `PATH` の `gh` の `gh pr view` だけで、引けなければ番号が付かないだけ |
