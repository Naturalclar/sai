...
| `GET /api/agent/sessions?from=` | **エージェント用**（#310。SAI の MCP サーバの `sai_sessions`）。送り元（`from`）と同じ project の、返信できる別のセッションの一覧 `{ "from", "sessions": [{ "id", "name", "project", "branch", "agent", "busy", "last_text" }] }`。本文は載せない。**`X-SAI-Agent-Token`（feed dir の `agent-token` の中身）が要り、`Origin` / `Sec-Fetch-Site` が付いていれば（ブラウザから）`403`**。送り元が SAI から起動していまターンを回していなければ `409` |
| `POST /api/agent/send` | エージェント用。body `{ "from", "to", "text" }`。見出し（`【SAI】#<project> の「<送り元>」からのメッセージです（id: …）`）を付けて `to` のターンを起動する（処理中なら預かり。#305）。`202` で `{ "message_id", "to", "via", "sent", "limit" }`。同じ project の返信できる別のセッションでなければ `403`、受け取ったメッセージで回っているターンから（連鎖）・1 ターン 3 回を超えたら `429`（#311）、本文が空か 4,000 字を超えたら `400` |
...
| `GET /api/sessions/<id>/progress?days=90` | 処理中のターンがいま何をしているか（#302）。`{ "rev", "id", "active", "steps", "total", "updated_at" }`。`steps` は最後のターンの直近の手順（古い順、最大 8 件。`kind` = `tool` / `thinking` / `text`、`tool`、`summary`、`started`、走っているツールには `ended` が無い）、`total` はそのターンの手順の数、`active` はターンが閉じておらず止まったまま古くなってもいないか。Claude は transcript（`~/.claude/projects/<cwd の記号を - に>/<session>.jsonl`）、Codex は rollout の**末尾だけを読む**（パスは行とホームから組み立て、リクエストからは受けない）。OpenCode・別のマシンのセッション・読めないときは空。3 秒のポーリングには乗せない（画面が処理中のセッションを出している間だけ取る） |
