...
詳しくは [docs/screen.md](screen.md) の「一言コメント」。思考つきのモデル（qwen3 など）には要求に `reasoning_effort: "none"` を付けて**思考を切る**（Ollama 0.33 の OpenAI 互換の口で効くのはこれだけ。`think: false` と `/no_think` は無視される）。切らないと 40 文字の一言のために本文の何倍もの思考を先に生成して 90 秒の timeout に掛かり、作り直しのあいだ GPU を占有して Mac 全体が重くなる（実測: 同じ依頼が思考あり 11 秒・416 トークン、無し 0.9 秒・30 トークン）。この指定を受けない口（OpenAI 本家の推論でないモデル、知らないキーを 422 で弾く厳格なサーバなど）が 4xx を返したら、外して 1 回だけ送り直し、通ればその口には付けずに送る（サーバの標準エラー＝起動したペインに 1 行出る。覚えているのは口を作り直すまで＝設定を変える・立て直すまで）。送り直すのは 400 / 422 だけ（401 / 404 は指定のせいではなく、408 / 429 は一時的なので、外して通っても覚えない）。外しても通らなければそのまま失敗。それでも `<think>…</think>` が返ってきたら落としてから使う。
