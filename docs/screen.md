...
**ボタンはトグル**で、出しているときにもう一度押すと閉じる（✕ と Esc でも閉じる。別のセッションやフィードへ移ると閉じる）。**差分が何も無いセッションではボタンごと出ない。** フィードには出ない（差分のペインは開いているセッションのものなので、押しても行き先が無い）。ペインでもモーダルでも**流れるのは中身だけ**で、題名と ✕ は上に出たまま（#221。前はモーダル全体が流れたので、閉じるは差分の一番下にあった）。
