...
| Claude | **上限に当たっているときだけ**（`Claude 上限中`）と復帰時刻 | `~/.claude/projects/*/*.jsonl` の `quotaLimits`。**弾かれたときにしか載らない**ので、平常時の使用率はローカルに無い |

- Claude の「いま何 % 使っているか」は**取れない**（`claude` CLI に `usage` のサブコマンドも無い）。取るには API を叩くことになるので、やらない。パネルにもその旨を書いてある
