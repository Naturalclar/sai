...
- **一言が変だと思ったら、その場で残せる**（#346）。一言の横の「変？」を押すと理由（意味が変わった / 事実が違う / 長い / 前置き / 口調）を選べて、任意で「こうしてほしい」も書ける。**その場の一言は変わらない**（作り直さない）。`~/.agent-feed/digest-feedback.jsonl` に溜まり、規則を直すときの材料と回帰テストの素材になる。溜めたものは**外には出さない**
